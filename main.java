	...
	public static void main(String[] args) {
		...
		while (menu <= 1 || menu >= 6) {
			...
			for (index = 0; index <= (infoContact.size() - 1); index++) {
				System.out.println(index + 1 + " -- " + infoContact.get(index).getName() + " -- " + infoContact.get(index).getNumber() + " -- " + infoContact.get(index).getAddress());
			}
			System.out.println();
			...
			case 1:
				...
				System.out.println("Qual é o endereço da pessoa?\n");
			...
			case 3:
				if (infoContact.size() > 0) {
					...
					if (comunication <= 0 || comunication >= (infoContact.size() - 1)) {
						System.out.println("Olá " + infoContact.get(comunication - 1).getName() + " como está? Tudo bem?");
					}
				}
				menu = -1;
				System.out.println();
